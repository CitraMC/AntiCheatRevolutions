...
public class CommandReport extends CommandBase {
	...
	private static final String NAME = "AntiCheatRevolutions Reports";
	...
	protected void execute(CommandSender cs, String[] args) {
		if (args.length >= 1) {
			...
			if ("low".equalsIgnoreCase(args[0])) {
				...
				User user = AntiCheatRevolutions.getManager().getUserManager().getUser(targetPlayer.getUniqueId());
				if (user != null) {
