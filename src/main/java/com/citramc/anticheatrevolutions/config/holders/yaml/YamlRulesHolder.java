...
public class YamlRulesHolder extends ConfigurationFile implements Rules {
    ...
    public YamlRulesHolder(AntiCheatRevolutions plugin, Configuration config) {
    ...
    public void open() {
        ...
        for (int i = 0; i < tempRules.size(); i++) {
            ...
            if (rule != null) {
                ...
                AntiCheatRevolutions.getPlugin().getLogger().warning("Couldn't load rule '" + string + "' from config. Improper format used.");
            }
        }
    }
}
