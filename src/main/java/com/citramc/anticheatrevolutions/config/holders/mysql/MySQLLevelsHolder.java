        ...
        Bukkit.getScheduler().runTaskAsynchronously(AntiCheatRevolutions.getPlugin(), new Runnable() {
            ...
            public void run() {
                if (!user.isWaitingOnLevelSync()) {
                    AntiCheatRevolutions.debugLog("Saving level from " + user.getUUID() + ". Value: " + user.getLevel());
                    try {
                ...
                AntiCheatRevolutions.getManager().getUserManager().removeUser(user);
            }
        });
    }
        ...
        Bukkit.getScheduler().runTaskAsynchronously(AntiCheatRevolutions.getPlugin(), new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    while (set.next()) {
                        has = true;
                        AntiCheatRevolutions.debugLog("Syncing level to " + user.getUUID() + ". Value: " + set.getInt("level"));
