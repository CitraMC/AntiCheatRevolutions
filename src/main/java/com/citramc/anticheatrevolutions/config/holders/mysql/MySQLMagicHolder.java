...
package com.citramc.anticheatrevolutions.config.holders.mysql;
...
import com.citramc.anticheatrevolutions.config.ConfigurationTable;
...
public class MySQLMagicHolder extends ConfigurationTable implements InvocationHandler {
    ...
    @Override
    public void open() {
    	InputStreamReader reader = new InputStreamReader(AntiCheatRevolutions.getPlugin().getResource("magic.yml"));
        ...
        try {
            ...
            while (set.next()) {
                ...
                if (set.getObject("value_int") != null) {
                    ...
                    AntiCheatRevolutions.getPlugin().getLogger().severe("The magic value " + key + " loaded from the database did not have a value configured. Using the default value.");
                    for (int i=1;i<=methods.length;i++) {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
        ...
        AntiCheatRevolutions.getPlugin().getLogger().severe("The magic value " + key + " couldn't be found.");
        return 0;
    }
}
