...
package com.citramc.anticheatrevolutions.config;
...
import com.citramc.anticheatrevolutions.config.files.Config;
import com.citramc.anticheatrevolutions.config.files.Enterprise;
import com.citramc.anticheatrevolutions.config.holders.mysql.MySQLGroupsHolder;
import com.citramc.anticheatrevolutions.config.holders.mysql.MySQLLangHolder;
import com.citramc.anticheatrevolutions.config.holders.mysql.MySQLLevelsHolder;
import com.citramc.anticheatrevolutions.config.holders.mysql.MySQLMagicHolder;
import com.citramc.anticheatrevolutions.config.holders.mysql.MySQLRulesHolder;
import com.citramc.anticheatrevolutions.config.holders.yaml.YamlChecksHolder;
import com.citramc.anticheatrevolutions.config.holders.yaml.YamlGroupsHolder;
import com.citramc.anticheatrevolutions.config.holders.yaml.YamlLangHolder;
import com.citramc.anticheatrevolutions.config.holders.yaml.YamlLevelsHolder;
import com.citramc.anticheatrevolutions.config.holders.yaml.YamlMagicHolder;
import com.citramc.anticheatrevolutions.config.holders.yaml.YamlRulesHolder;
import com.citramc.anticheatrevolutions.config.providers.Checks;
import com.citramc.anticheatrevolutions.config.providers.Groups;
import com.citramc.anticheatrevolutions.config.providers.Lang;
import com.citramc.anticheatrevolutions.config.providers.Levels;
import com.citramc.anticheatrevolutions.config.providers.Magic;
import com.citramc.anticheatrevolutions.config.providers.Rules;
...
public class Configuration {
    ...
    public Configuration(AntiCheatRevolutions plugin, AntiCheatManager manager) {
        removeOldFiles();
    ...
    private void removeOldFiles() {
        ...
        File configFile = new File(AntiCheatRevolutions.getPlugin().getDataFolder(), "config.yml");
        if (configFile.exists() && YamlConfiguration.loadConfiguration(configFile).getString("System.Auto update") != null) {
            configFile.renameTo(new File(AntiCheatRevolutions.getPlugin().getDataFolder(), "config.old"));
            removed.add("config.yml has been renamed to config.old and replaced with the new config.yml");
        }
        File eventsFile = new File(AntiCheatRevolutions.getPlugin().getDataFolder(), "events.yml");
        if (eventsFile.exists()) {
            eventsFile.renameTo(new File(AntiCheatRevolutions.getPlugin().getDataFolder(), "events.old"));
            removed.add("events.yml has been renamed to events.old and replaced with groups.yml and rules.yml");
        }
        if (removed.size() > 0) {
            AntiCheatRevolutions.getPlugin().getLogger().info("You are upgrading from an old version of AntiCheat. Due to configuration changes, the following files have been modified:");
            for (String s : removed) {
                AntiCheatRevolutions.getPlugin().getLogger().info(s);
            }
        }
    }
